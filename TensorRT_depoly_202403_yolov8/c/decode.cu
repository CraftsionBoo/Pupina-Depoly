...
namespace v8
{
    ...
    static __global__ void fast_nms_kernel(float *bboxes, int MAX_IMAGE_BOXES, float threshold, int NUM_BOX_ELEMENT)
    {
        int position = (blockDim.x * blockIdx.x + threadIdx.x);
    ...
    void decode_kernel_invoker(float *predict, int num_bboxes, int num_classes, int output_cdim,
                               ...
                               float *invert_affine_matrix, float *parray, int MAX_IMAGE_BOXES, int NUM_BOX_ELEMENT, Task task, cudaStream_t stream)
    {
        auto grid = Taurus::cudaTools::grid_dims(num_bboxes);
        auto block = Taurus::cudaTools::block_dims(num_bboxes);

        if (task == Task::det || task == Task::seg)
        {
            checkCUDAKernel(decode_kernel_v8<<<grid, block, 0, stream>>>(
        ...
        grid = Taurus::cudaTools::grid_dims(MAX_IMAGE_BOXES);
        block = Taurus::cudaTools::block_dims(MAX_IMAGE_BOXES);
        checkCUDAKernel(fast_nms_kernel<<<grid, block, 0, stream>>>(parray, MAX_IMAGE_BOXES, nms_threshold, NUM_BOX_ELEMENT));
    }
