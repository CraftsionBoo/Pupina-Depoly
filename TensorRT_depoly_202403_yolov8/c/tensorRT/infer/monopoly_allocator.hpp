#ifndef __MONOPOLY_ALLOCATOR_HPP__
#define __MONOPOLY_ALLOCATOR_HPP__

/** 独占分配器
 * 1.实现tensor复用问题
 * 2.对于tensor使用两个阶段实现并行，时间重叠
 * **/
...
public:
    ...
    /**
     * 获取一个可用对象
     * timeout 超时时间，如果没有可用对象，将会进入阻塞等待，等待超时返回空空指针
     * 请求等到一个对象后，该对象被占用，除非执行了release释放该对象所有权
     * **/
...
private:
    ...
    int capacity_ = 0;
    volatile int num_available_ = 0; // volatile指示编译器一个变量值可以在外部修改
