#ifndef __INFER_CONTROLLER_HPP__
#define __INFER_CONTROLLER_HPP__
...
public:
    struct Job
    {
        ...
        MonopolyAllocator<Taurus::TRT::Tensor>::MonopolyDataPointer mono_tensor;
...
protected:
    ...
    virtual bool get_jobs_and_wait(std::vector<Job> &fetech_jobs, int max_size)
    {
        std::unique_lock<std::mutex> l(jobs_lock_);
        cond_.wait(l, [&]()
        ...
        {
            fetech_jobs.emplace_back(std::move(jobs_.front()));
            jobs_.pop();
        }
        return true;
    }

    virtual bool get_jobs_and_wait(Job &fetch_job)
    {
        std::unique_lock<std::mutex> l(jobs_lock_);
        cond_.wait(l, [&]()
                   { return !run_ || !jobs_.empty(); });
...
protected:
    ...
    std::shared_ptr<MonopolyAllocator<Taurus::TRT::Tensor>> tensor_allocator_;
};
