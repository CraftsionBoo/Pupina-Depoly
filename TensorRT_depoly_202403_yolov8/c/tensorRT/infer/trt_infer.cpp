    ...
    namespace TRT
    {
        ...
        public:
            ...
            bool build_model(const void *pdata, size_t size)
            {
                ...
                engine_ = std::shared_ptr<nvinfer1::ICudaEngine>(runtime_->deserializeCudaEngine(pdata, size, nullptr), destroy_nvidia_pointer<nvinfer1::ICudaEngine>);
                if (engine_ == nullptr)
                    return false;

                context_ = std::shared_ptr<nvinfer1::IExecutionContext>(engine_->createExecutionContext(), destroy_nvidia_pointer<nvinfer1::IExecutionContext>);
        ...
        public:
            ...
            virtual std::shared_ptr<Memory> get_workspace() override;
        ...
        void InferImpl::print()
        {
            ...
            INFOI("Infer %p detail", this);
            INFOI("\tBase device: %s", cudaTools::description().c_str());
            INFOI("\tMax Batch Size: %d", this->get_max_batch_size());
            INFOI("\tInputs: %d", inputs_.size());
            for (int i = 0; i < inputs_.size(); ++i)
            {
                auto &tensor = inputs_[i];
                auto &name = inputs_name_[i];
                INFOI("\t\t%d.%s : shape {%s}, %s", i, name.c_str(), tensor->shape_string(), data_type_string(tensor->type()));
            }

            INFOI("\tOutputs: %d", outputs_.size());
        ...
        {
            auto data = cUtils::load_file(file);
            if (data.empty())
                return false;
            context_.reset(new EngineContext());
        ...
        void InferImpl::forward(bool sync)
        {
            ...
            bool execute_result = context->context_->enqueueV2(bingdingPtr, context->stream_, nullptr);
            if (!execute_result)
            {
        ...
        std::shared_ptr<Memory> InferImpl::get_workspace()
        {
            return workspace_;
        }
