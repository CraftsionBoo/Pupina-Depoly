...
#define CAST_BITS (INTER_RESIZE_COEF_BITS << 1)
        ...
        Norm Norm::alpha_bata(float alpha, float beta, ChannelType channel_type)
        {
            Norm out;
        ...
        __global__ void resize_bilinear_kernel(uint8_t *src, int src_line_size,
        ...
        __global__ void warp_affine_bilinear_and_normalize_focus_kernel(uint8_t *src, int src_line_size, int src_width, int src_height,
                                                                        float *dst, int dst_width, int dst_height,
                                                                        uint8_t const_value_st, float *warp_affine_matrix_1_3,
                                                                        Norm norm, int edge)
        {
            int position = blockDim.x * blockIdx.x + threadIdx.x;
            if (position > edge)
            ...
            cudaStream_t stream)
        {
            ...
            checkCUDAKernel(warp_affine_bilinear_and_normalize_plane_kernel<<<grid, block, 0, stream>>>(
                ...
                const_value, matrix_2_3, norm, jobs));
        }
            ...
            cudaStream_t stream)
        {
            ...
            checkCUDAKernel(warp_affine_bilinear_and_normalize_focus_kernel<<<grid, block, 0, stream>>>(
                src, src_line_size, src_width, src_height,
                dst, dst_width, dst_height,
                const_value, matrix_1_3, norm, jobs));
        }
