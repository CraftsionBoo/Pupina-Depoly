...
#include "tensorRT/infer/infer_controller.hpp"
#include "tensorRT/infer/monopoly_allocator.hpp"
...
namespace v8
{
    ...
    void decode_kernel_invoker(float *predict, int num_bboxes,
                               ...
                               int max_objects, int num_box_element, Task task,
                               cudaStream_t stream);
    ...
    public:
        ...
        virtual bool startup(
            const std::string &file, Task task,
            int gpuid,
        ...
        virtual void worker(std::promise<bool> &result) override
        {
            ...
            TRT::Tensor output_segment_device(TRT::DataType::Float32);
            ...
            if (this->task_ == Task::seg)
            {
                ...
                segment_head_dims_ = engine->tensor("output1");
                num_classes_ = bbox_head_dims_->size(2) - 4 - segment_head_dims_->size(1);
            }
            else if (this->task_ == Task::det)
            {
                bbox_head_dims_ = engine->tensor("output0");
                num_classes_ = bbox_head_dims_->size(2) - 4;
            }
            ...
            while (get_jobs_and_wait(fetch_jobs, max_batch_size))
            {
                ...
                for (int ibatch = 0; ibatch < infer_batch_size; ++ibatch)
                {
                    ...
                    decode_kernel_invoker(image_based_output, bbox_head_dims_->size(1),
                                          num_classes_, bbox_head_dims_->size(2),
                                          confidence_threshold_, nms_threshold_,
                                          affine_matrix, output_array_ptr,
                                          MAX_IMAGE_BBOX, NUM_BOX_ELEMENT,
                                          Task::seg, stream_);
                }
                        ...
                        if (keepflag == 1)
                        {
                            ...
                            if (task_ == Task::seg)
                            {
                                ...
                                if (mask_out_width > 0 && mask_out_height > 0)
                                {
                                    ...
                                    std::shared_ptr<InstanceSegmentMap> mask_out_host = std::make_shared<InstanceSegmentMap>(mask_out_width, mask_out_height);
                                    result_object_box.seg = std::make_shared<InstanceSegmentMap>(pbox[2] - pbox[0], pbox[3] - pbox[1]);
                                    ...
                                    checkCudaRuntime(cudaMemcpyAsync(mask_out_host->data, mask_out_device, bytes_of_mask_out, cudaMemcpyDeviceToHost, stream_));
    ...
    private:
        ...
        NMSMethod nms_method_ = NMSMethod::CPU;
    ...
    std::shared_ptr<SegInfer> create_seg_infer(
        const std::string &engine_file, Task task, int gpuid,
        float confidence_threshold, float nms_threshold,
        ...
        bool use_multi_preprocess_stream)
    {
        ...
        if (!instance->startup(engine_file, task, gpuid, confidence_threshold,
